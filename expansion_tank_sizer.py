...
def ASHRAE_sizing_method(max_temp, sys_vol, min_tank_psig, max_tank_psig):
    """ASHRAE's Equation for determining the expansion tank volume for a diaphragm tank. Assumes a 40°F fill temp."""
    return (0.00041 * max_temp - 0.0466) * sys_vol / (
...
def critical_sizing_method(sys_vol, init_temp, max_temp, min_tank_psig, max_tank_psig):
    ...
    return sys_vol * net_exp_factor / accept_factor, net_exp_factor, accept_factor
...
def calculate_tank_volumes():
    ...
    # ASHRAE method
    ashrae_volume = ASHRAE_sizing_method(max_temp, sys_vol, min_tank_psig, max_tank_psig)

    # Critical method
    critical_result = critical_sizing_method(sys_vol, init_temp, max_temp, min_tank_psig, max_tank_psig)

    if isinstance(critical_result, tuple):
        critical_volume, net_exp_factor, accept_factor = critical_result
        critical_output = (
            f"Critical Method Min Tank Volume: {critical_volume:.2f} gallons\n"
            f"Net Expansion Factor: {net_exp_factor:.4f}\n"
            f"Acceptance Factor: {accept_factor:.4f}"
        )
    else:
        critical_output = f"Critical Method Error: {critical_result}"
...
temp_frame = tk.LabelFrame(root, text="Temperature Inputs", padx=10, pady=10)
temp_frame.pack(padx=10, pady=5, fill="x")

max_temp_var = tk.StringVar()
max_temp_options = [120, 130, 140, 150, 160, 170, 180, 190, 200, 210, 220, 230, 240]

add_row(temp_frame, "Max Water Temp (°F):",
        ttk.Combobox(temp_frame, textvariable=max_temp_var, values=max_temp_options, state="readonly"), row=0)
...
for frame in [shared_frame, temp_frame, critical_frame]:
